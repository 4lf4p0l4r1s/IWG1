...
from .models import RegistroKilometraje
# En usuarios/views.py
from jueguitoAutos.forms import RegistroKilometrajeForm, UsuarioForm
...
def registrar_kilometraje(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                registro_existente = RegistroKilometraje.objects.filter(usuario=usuario,
                                                                            fecha=semana_actual).first()
